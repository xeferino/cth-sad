			...
			<div class="login-form">
                <form method="POST" name="form-login-reset" id="form-login-reset">
                    ...
                    <div class="form-action mb-3">
                      <button type="submit" class="btn btn-secondary btn-block" id="login"> Enviar</button>
                    </div>
        ...
        $("#form-login-reset").submit(function( event ) {
            ...
            if(email==""){
                ...
                $.notify({
                        icon: 'flaticon-error',
                        title: 'Error!',
                        message: '<p style="font-size: 14px;">El email es obligatorio.</p>',
                    },
                    {
                        type: 'danger',
                        placement: {
                ...
                $('#login').prop("disabled", false).text("Enviar");
                setTimeout(function () { $('#avatar').attr('src', '{{ asset('img/avatar.svg') }}'); }, 1000);
            }else{
                axios.post('{{ route('password')}}', {
                    email:email
                }).then(response => {
