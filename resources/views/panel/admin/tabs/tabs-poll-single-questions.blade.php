    ...
    <table align="center" width="100%" cellpadding="0" cellspacing="0" style="border: none !important;">
        @foreach ($cantons as $tab)
            <tr style="text-align: left; font-weight:bold;">
                <td colspan="14"><h5 style="text-align: center; font-weight:bold;"><img style="text-align: left; padding-bottom:5px;" src="{{ asset('img/polling.svg') }}" width="5%" alt=""><br><span style="font-size:20px;">TDATOS</span><br><br> {{ $tab['poll'] }} <br> Codigo: {{ $tab['prefix'] }}</h5></td>
            </tr>
            <tr style="text-align: left">
                <td colspan="14"><h5 style="text-align: center; font-weight:bold;">TABULACIÓN DE ENCUESTAS DE  SATISFACCIÓN DEL CONSUMIDOR PARA USUARIOS RESIDENCIALES Y COMERCIALES: {{ $tab['canton'] }} <br><br> PERIODO: {{ $tab['period'] }} </h5></td>
            </tr>
            @if ($tab['sections'])
            @php  $i=1; @endphp
            @php  $j=1; @endphp
                            ...
                            @else
                                ...
                                @if ($question['options'])
                                    ...
                                    @foreach ($question['options'] as $option)
                                        <tr style="text-align: center; border: solid #000 0.5px !important;">
                                            <td colspan="5" width="50%" style="text-align: center; font-weight:bold; border: solid #000 0.5px !important;"><h4 style="text-align: center; font-weight:bold;">{{ $option['name'] }}</h4></td>
                                            <td colspan="9" width="50%" style="text-align: left; margin-left:10px; font-size:12px font-weight:bold; border: solid #000 0.5px !important;">
...
@foreach ($cantons as $tab)
    @if ($tab['sections'])
        ...
        @php $porc =0; @endphp
        @foreach ($tab['sections'] as $section)
        @php $sectdata [] = sprintf('%0.2f', $section['percent_sect']); @endphp
            @php $sections [] = $section['id']; @endphp
            @php $porc +=$section['percent_sect']; @endphp
